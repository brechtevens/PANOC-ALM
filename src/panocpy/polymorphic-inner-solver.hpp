...
auto InnerSolverFullCallWrapper() {
    return [](InnerSolver &solver, const pa::ProblemFull &p, pa::crvec Σ1,
              pa::crvec Σ2, pa::real_t ε, pa::vec x,
              pa::vec y) -> std::tuple<pa::vec, pa::vec, pa::vec, pa::vec, py::dict> {
        pa::vec z1(p.m1);
        pa::vec z2(p.m2);
...
template <>
struct InnerStatsAccumulator<pa::PANOCSolverFull<>> {
    std::shared_ptr<PolymorphicInnerSolverStatsAccumulatorBase> ptr;
    py::dict to_dict() const { return ptr->to_dict(); }
};
...
template <class DirectionProviderT>
struct is_PANOCSolver<PANOCSolverFull<DirectionProviderT>> : std::true_type {};
template <class T>
...
inline py::dict stats_to_dict(pa::ALMSolverFull<>::Stats &s) {
    using py::operator""_a;
    return py::dict{
        "outer_iterations"_a           = s.outer_iterations,
        "elapsed_time"_a               = s.elapsed_time,
